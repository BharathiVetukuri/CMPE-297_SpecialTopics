...
async def analyze_code_structure(code: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use asyncio.to_thread for CPU-bound tasks instead of manual executors
        tree = await asyncio.to_thread(ast.parse, code)
        analysis = await asyncio.to_thread(_extract_code_structure, tree, code)
        ...
        return {
            ...
            "analysis": analysis,
            "summary": (
                f"Found {analysis['metrics']['function_count']} functions and "
                f"{analysis['metrics']['class_count']} classes"
            ),
        }
